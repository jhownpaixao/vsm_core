 ...
 */
class VSMLegacyRestorationQueue extends VSMBulkProcessingQueue
{
    protected bool                              m_ForceSpawnOnGround;
    protected ref array<ref VSMVirtualObject>   m_ProcessedItems;
    ...
    override bool OnInit()
    {
        ...
        m_ItemCount =           m_Metadata.m_Items.Count();
    ...
    {
        //! preparação
        VSMMetadataItem vItem = m_Metadata.m_Items.Get(idx);
        VSMVirtualObject obj = new VSMVirtualObject(m_VirtualContextDirectory, m_Container, vItem);
        m_ProcessedItems.Insert(obj);

        //! restauração
        ItemBase restored = obj.Restore(m_Ctx, m_StoreCtx, m_ForceSpawnOnGround);
    ...
    override void OnComplete()
    {
        ...
        for (int i = 0; i < m_ProcessedItems.Count(); i++)
		{
        ...
        //! o contexto deve ser salvo aqui, pois obj.OnRestoreComplete(); ainda pode fazer uso.
        m_Ctx.Close();
        m_StoreCtx.Close();
